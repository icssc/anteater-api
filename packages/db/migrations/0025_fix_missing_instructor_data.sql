...
ALTER TABLE "websoc_instructor" ADD COLUMN "identifier" varchar;--> statement-breakpoint
...
CREATE MATERIALIZED VIEW "public"."instructor_view" AS (with "shortened_names_cte" as (select "instructor_ucinetid", ARRAY_AGG(
                              DISTINCT split_part(
                                "websoc_instructor_name",
                                '&|*',
                                1
                              )
                            ) as "shortened_names" from "instructor_to_websoc_instructor" group by "instructor_to_websoc_instructor"."instructor_ucinetid"), "courses_cte" as (with "terms_cte" as (select "course"."id", "instructor_to_websoc_instructor"."instructor_ucinetid", 
