...
export type WebsocSectionMeeting =
	| TBAWebsocSectionMeeting
	| ConfirmedWebsocSectionMeeting;
...
export const sampleProgramVariation = pgTable("sample_program_variation", {
	...
	label: varchar("label"),
	sampleProgram: jsonb("sample_program")
		.$type<SampleProgramEntry[]>()
		.notNull(),
	variationNotes: varchar("variation_notes")
		.array()
		.notNull()
...
export const websocSectionType = pgEnum(
	"websoc_section_type",
	websocSectionTypes,
);
...
export const websocDepartment = pgTable(
	"websoc_department",
	{
		...
		sectionCodeRangeComments: text("section_code_range_comments")
			.array()
			.notNull(),
		courseNumberRangeComments: text("course_number_range_comments")
			.array()
			.notNull(),
	},
	(table) => [
...
export const websocCourse = pgTable(
	"websoc_course",
	{
		...
		courseId: varchar("course_id")
			...
			.generatedAlwaysAs(
				(): SQL =>
					sql`REPLACE(${websocCourse.deptCode}, ' ', '') || ${websocCourse.courseNumber}`,
			),
		updatedAt: timestamp("updated_at", {
			mode: "date",
			withTimezone: true,
...
export const websocSectionMeeting = pgTable(
	"websoc_section_meeting",
	{
		...
		timeIsTBA: boolean("time_is_tba")
			.notNull()
			.generatedAlwaysAs(
				(): SQL => sql`${websocSectionMeeting.timeString} LIKE \'%TBA%\'`,
			),
...
export const websocSectionEnrollment = pgTable(
	...
	(table) => [
		...
		uniqueIndex().on(table.sectionId, table.createdAt),
	],
);
...
export const course = pgTable(
	"course",
	{
		...
		shortenedDept: varchar("shortened_dept")
			.notNull()
			.generatedAlwaysAs(
				(): SQL => sql`REPLACE(${course.department}, ' ', '')`,
			),
		...
		prerequisiteTree: json("prerequisite_tree")
...
export const instructorToWebsocInstructor = pgTable(
	...
	{
		id: uuid("id").primaryKey().defaultRandom(),
		instructorUcinetid: varchar("instructor_ucinetid").references(
			() => instructor.ucinetid,
		),
...
export const majorSpecializationToRequirement = pgTable(
	"major_specialization_to_requirement",
	{
		majorId: varchar("major_id")
			.notNull()
			.references(() => major.id),
		specializationId: varchar("specialization_id").references(
			() => specialization.id,
		),
	...
	(table) => [
		unique().on(table.majorId, table.specializationId).nullsNotDistinct(),
	],
);
...
export const major = pgTable(
	...
	(table) => [
		index().on(table.degreeId),
		index().on(table.collegeRequirementId),
	],
);
...
export const apExamReward = pgTable("ap_exam_reward", {
	...
	coursesGranted: json("courses_granted")
		.$type<APCoursesGrantedTree>()
		.notNull(),
});
...
export const instructorView = pgMaterializedView("instructor_view").as((qb) => {
	const shortenedNamesCte = qb.$with("shortened_names_cte").as(
		qb
			.select({
				...
				shortenedNames:
					sql`ARRAY_AGG(${instructorToWebsocInstructor.websocInstructorName})`.as(
						"shortened_names",
					),
			})
			.from(instructorToWebsocInstructor)
	...
	return qb
		...
		.select({
			...
			shortenedNames: sql<
				string[]
			>`COALESCE(${shortenedNamesCte.shortenedNames}, ARRAY[]::TEXT[])`.as(
				"shortened_names",
			),
			courses: sql`
		...
		.leftJoin(
			shortenedNamesCte,
			eq(shortenedNamesCte.instructorUcinetid, instructor.ucinetid),
		)
		.leftJoin(
			coursesCte,
			eq(coursesCte.instructorUcinetid, instructor.ucinetid),
		)
