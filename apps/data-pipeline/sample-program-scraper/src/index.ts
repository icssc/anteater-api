...
async function fetchSchoolsPath(): Promise<string[]> {
	const schoolsAndPrograms = await fetchWithDelay(
		`${CATALOGUE_URL}/schoolsandprograms/`,
	);
...
async function lookupCourseId(
	...
	try {
		...
		const [foundCourse] = await db
			...
			.where(
				and(
					eq(course.department, parsed.dept),
					eq(course.courseNumber, parsed.num),
				),
			)
			.limit(1);
		...
		for (const variation of program.variations) {
			...
			for (const year of variation.sampleProgram) {
				...
				const [fall, winter, spring] = await Promise.all([
					...
					Promise.all(
						year.winter.map((code) => interpretCourseString(db, code)),
					),
					Promise.all(
						year.spring.map((code) => interpretCourseString(db, code)),
					),
				]);
...
async function storeSampleProgramsInDB(
	...
	const transformedPrograms = await transformCourseCodesToIds(
		db,
		scrapedPrograms,
	);
	...
	const sortedDbData = sortKeys(
		{
			catalogPrograms: dbData.catalogPrograms.sort((a, b) =>
				a.id.localeCompare(b.id),
			),
			variations: dbData.variations.sort((a, b) =>
	...
	const sortedScrapedData = sortKeys(
		{
			catalogPrograms: scrapedData.catalogPrograms.sort((a, b) =>
				a.id.localeCompare(b.id),
			),
			variations: scrapedData.variations.sort((a, b) =>
				a.programId.localeCompare(b.programId),
			),
		},
		{ deep: true },
	...
	if (!foundYearHeader && currentCurriculum.length > 0) {
		...
		if (prevHeading.length) {
			...
			if (headingText.match(/freshmen|freshman/i)) {
				...
				logger.warn(
					`Unrecognized year text: "${headingText}", defaulting to Freshman`,
				);
				yearText = "Freshman";
			}
		}
...
function parseFootnotes(
	...
	const dlElements = element.is("dl.sc_footnotes")
		? element
		: element.find("dl.sc_footnotes");
	...
	dlElements.each((_i, dl_el) => {
		$(dl_el)
			...
			.each((_j, dd_el) => {
				const noteText =
					$(dd_el).find("p").text().trim() || $(dd_el).text().trim();
...
async function scrapeSamplePrograms(programPath: string) {
	...
	const programName = $("h1.page-title")
		.text()
		.normalize("NFKD")
		.split("(")[0]
		.trim();
		...
		if (sampleYears.length > 0) {
			...
			if (variationNotes.length === 0) {
				sampleProgramContainer.find("p").each((_i, p_el) => {
					const pText = $(p_el).text().trim();
					if (
						(pText.startsWith("*") || pText.match(/^\d+\./)) &&
	...
	// CASE 2: Multiple Tables - Labels Required
		...
		allTables.each((index, tableEl) => {
			...
			if (prevP.length && prevP.text().trim()) {
				...
				if (
					!pText.match(
						/^Items in parentheses are recommended choices or alternatives/i,
					)
				) {
					label = pText;
				}
			...
			if (sampleYears.length > 0) {
				...
				while (
					nextElement.length &&
					!nextElement.is("table, h1, h2, h3, h4, h5, h6")
				) {
				...
				logger.info(
					`Found ${variationNotes.length} notes for variation "${label}"`,
				);
