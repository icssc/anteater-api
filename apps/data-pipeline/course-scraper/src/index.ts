...
const getDepartmentAlias = (dept: string) =>
	orNull(DEPT_TO_ALIAS[dept as DeptCode]);
...
function parseCourseBlock(
	...
	const { $, deptCode, schoolName, departmentName, prereqs, updatedAt } =
		context;
	...
	const courseNumber = code.startsWith(deptCode)
		? code.slice(deptCode.length).trim()
		: code;
	...
	const prereqText = textAfterLabel(
		$b.find(".detail-prereqs").first().text(),
		"Prerequisite",
	);
	...
	const sameAsText = textAfterLabel(
		$b.find(".detail-same_as").first().text(),
		"Same as",
	);
	...
	const restrText = textAfterLabel(
		$b.find(".detail-restrictions").first().text(),
		"Restriction",
	);
...
async function fetchWithDelay(url: string, delayMs = 1000) {
	try {
		...
		await sleep(delayMs);
		const res = await fetch(url, { headers: HEADERS_INIT }).then((x) =>
			x.text(),
		);
...
function parsePrerequisite(prereq: string): Prerequisite | undefined {
	const reqWithGradeMatch = prereq.match(
		/^([^()]+)\s+\( min (\S+) = (\S{1,2}) \)$/,
	);
...
function parseAntirequisite(prereq: string): Prerequisite | undefined {
	const antiAPReqMatch = prereq.match(
...
function buildPrereqTree(prereqList: string): PrerequisiteTree {
	...
	if (prereqTree.AND) {
		if (
			!prereqTree.NOT &&
			prereqTree.AND.length === 1 &&
			"OR" in prereqTree.AND[0]
		) {
...
async function scrapePrerequisitePage(deptCode: string, url: string) {
	...
	$("table tbody tr").each(function () {
		...
		if ($(entry).length === 3) {
			let courseId = $(entry[prereqFieldLabels.Course])
				.text()
				.replace(/\s+/g, " ")
				.trim();
			const courseTitle = $(entry[prereqFieldLabels.Title])
				.text()
				.replace(/\s+/g, " ")
				.trim();
...
const isPrereq = (x: Prerequisite | PrerequisiteTree): x is Prerequisite =>
	"prereqType" in x;

const prereqToString = (prereq: Prerequisite) =>
	prereq.prereqType === "course"
		? prereq.courseId.replaceAll(/ /g, "")
		: prereq.examName;

function prereqTreeToList(tree: PrerequisiteTree): string[] {
	if (tree.AND) {
		return tree.AND.flatMap((x) =>
			isPrereq(x) ? prereqToString(x) : prereqTreeToList(x),
		);
	}
	if (tree.OR) {
		return tree.OR.flatMap((x) =>
			isPrereq(x) ? prereqToString(x) : prereqTreeToList(x),
		);
	}
	return [];
...
async function scrapeCoursesInDepartment(meta: {
	...
	if (!prereqs) {
		...
		logger.warn(
			"Prerequisite data for courses in this department will be empty.",
		);
	}
	...
	const departmentName = $("h1.page-title")
		.text()
		.normalize("NFKD")
		.split("(")[0]
		.trim();
	...
	if (!courseDiff.length) {
		logger.info(
			`No difference found between database and scraped course data for ${deptCode}.`,
		);
	} else {
		console.log(
			`Difference between database and scraped course data for ${deptCode}:`,
		);
	...
	if (!prereqDiff.length) {
		...
		console.log(
			`Difference between database and scraped prerequisite data for ${deptCode}:`,
		);
...
async function patchH32(meta: { db: ReturnType<typeof database> }) {
	...
	const courses = await db
		...
		.then(
			(x) =>
				new Map(x.map(({ id, prerequisiteTree }) => [id, prerequisiteTree])),
		);
	for (const [id, prerequisiteTree] of courses) {
		const newPrereqTree = JSON.parse(
			JSON.stringify(prerequisiteTree),
		) as PrerequisiteTree;
		...
		if (newPrereqTree.AND.length === 1) {
			...
			if (!node32A || !nodeH32) {
				...
				(newPrereqTree.AND[0] as PrerequisiteTree).OR?.push({
					...originalNode,
					courseId:
						originalNode.courseId === "I&C SCI 32A"
							? "I&C SCI H32"
							: "I&C SCI 32A",
				});
			}
		...
		await db
			.update(course)
			.set({ prerequisiteTree: newPrereqTree })
...
async function main() {
	...
	if (existsSync("./prerequisites.json")) {
		...
		logger.info(
			`Found a prerequisite dump on disk (last modified ${stats.mtime}).`,
		);
		if (readlineSync.keyInYNStrict("Use this dump?")) {
			prerequisites = new Map(
				Object.entries(
					...
					k,
					new Map(Object.entries(v as Record<string, PrerequisiteTree>)),
				]),
			);
		...
		writeFileSync(
			...
			JSON.stringify(
				Object.fromEntries(
					prerequisites
						.entries()
						.map(([k, v]) => [k, Object.fromEntries(v.entries())]),
				),
			),
		);
	...
	const departments = new Map(
		$("#atozindex")
			...
			.flatMap((el) =>
				hasChildren(el) ? Object.values(el.children).filter(hasChildren) : [],
			)
			.map((el): [string, string] | undefined =>
				el.firstChild?.type === "tag" &&
				el.firstChild.firstChild?.type === "text"
					? [
							el.firstChild.firstChild.data.split("(")[1].slice(0, -1),
							el.firstChild.attribs.href,
						]
					: undefined,
			)
			.filter((entry) => !!entry),
