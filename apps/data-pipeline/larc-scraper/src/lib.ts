...
const quarterToLarcSuffix = (
	year: number,
	quarter: Exclude<Term, "Summer10wk">,
): string => {
	switch (quarter) {
		case "Fall":
...
const fmtBldg = (building: string): string =>
	building === "Online" ? "ON LINE" : building;
...
export async function doScrape(db: ReturnType<typeof database>) {
	...
	for (let year = EARLIEST_YEAR; year < new Date().getFullYear() + 2; ++year) {
		for (const quarter of quarters) {
			...
			data.push(
				...courses.map((course) => {
					const { startTime, endTime } = parseStartAndEndTimes(
						course.timeString,
					);
	...
	for (const k of courseToSections.keys()) {
		...
		const deptCode = fullCourseNumber
			.slice(0, fullCourseNumber.lastIndexOf(" "))
			.trim();
		const courseNumber = fullCourseNumber
			.slice(fullCourseNumber.lastIndexOf(" "))
			.trim();
