...
async function collectLocationMeta(): Promise<Record<string, LocationMeta>> {
	const html = await fetch(
		"https://www.lib.uci.edu/where-do-you-want-study-today",
	).then((r) => r.text());
	const $ = load(html);
	...
	if (!locMatch || !floorMatch)
		throw new Error("Unable to capture locationIds or floors array");
	...
	locationIds.forEach((id, idx) => {
		...
		const libraryLabel = locationSelection
			.find("h2.card-title")
			.first()
			.text()
			.trim();
...
async function fetchLocation(id: string): Promise<RawRespOK["data"] | null> {
	...
	try {
		...
		if (!validatedResponse.success) {
			console.error(
				`ID ${id} returned invalid response structure:`,
...
export async function doScrape(db: ReturnType<typeof database>) {
	...
	for (const [id, meta] of Object.entries(locationMeta)) {
		const libraryCode =
			libraryCodeMap[meta.libraryName as keyof typeof libraryCodeMap];
		...
		if (scrapeStatus === "idle" && currentMinute >= 15) {
			console.log(
				`Skipping ${meta.libraryName} (idle) - scraping only on the hour.`,
			);
			continue;
		}
	...
	await db
		.delete(libraryTrafficHistory)
		.where(lt(libraryTrafficHistory.timestamp, cutoff));
