...
export async function updateRestaurant(
	...
	const stationsToUpsert = Object.keys(restaurantInfo.stationsInfo).map(
		(id) => {
			return {
				id,
				restaurantId,
	...
	for (const dateToFetch of datesToFetch) {
		...
		const currentSchedule = findCurrentlyActiveSchedule(
	...
	const periodsInserted = await db
		...
		.onConflictDoUpdate({
			target: [
				diningPeriod.adobeId,
				diningPeriod.date,
				diningPeriod.restaurantId,
			],
	...
	await Promise.all(
		...
		Array.from(periodSet).map(async (periodAdobeId) => {
			...
			const currentPeriodWeekly = await fetchMenuWeekView(
				today,
				restaurantId,
				periodAdobeId,
			);
			...
			if (!currentPeriodWeekly) {
				console.log(
					`Skipping period ${periodAdobeId}, period dishes field is null.`,
				);
				return;
			}
	...
	if (dishBundlesToInsert.length > 0) {
		console.log(
			`Processing ${dishBundlesToInsert.length} dish bundles into upserts...`,
		);

		const dishesToUpsert = new Map<string, typeof diningDish.$inferInsert>();
		const dietRestrictionsToUpsert = new Map<
			string,
			typeof diningDietRestriction.$inferInsert
		>();
		const nutritionInfosToUpsert = new Map<
			string,
			typeof diningNutritionInfo.$inferInsert
		>();
		...
		for (const {
			...
			for (const key of allergens) {
				...
				baseDietRestriction[containsKey] =
					recipeAllergenCodes.has(allergenCode);
			}
		...
		await db.transaction(async (tx) => {
			...
			console.log(
				`Upserting ${dietRestrictionsToUpsert.size} dietary restriction entries...`,
			);
			await tx
			...
			console.log(
				`Upserting ${nutritionInfosToUpsert.size} nutrition info entries...`,
			);
			await tx
