...
async function scrapePlazaVerde(): Promise<{
	...
	const allStaffIds = [
		...new Set(services.flatMap((service) => service.staffMemberIds)),
	];
	...
	const availabilities = await fetchStaffAvailability(
		allStaffIds,
		startDateTime,
		endDateTime,
	);
...
export async function doPVScrape(db: ReturnType<typeof database>) {
	...
	await db.transaction(async (tx) => {
		...
		await tx
			.delete(studyRoomSlot)
			.where(inArray(studyRoomSlot.studyRoomId, roomIds));
