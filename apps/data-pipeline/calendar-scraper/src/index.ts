...
async function getTermDateData(year: string): Promise<TermDateData[]> {
	...
	const soc: Record<string, Date> = Object.fromEntries(
		[
			...data.slice(socIdx, socIdx + 3),
			...data.slice(socSummerIdx, socSummerIdx + 3),
		].map((x, i) => [term(i), parseDate(yearNum + Number(i > 1), x)]),
	);
...
async function main() {
	...
	if (!diff.length) {
		console.log(
			"No difference found between database and scraped calendar term data.",
		);
