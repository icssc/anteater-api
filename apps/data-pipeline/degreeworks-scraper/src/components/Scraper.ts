...
export class Scraper {
			...
			.filter(
				([_k, v]) =>
					...
					this.transformDegreeShort(
						awardTypesMap.get(catalogueDegree.degree.degreeCode as string)
							?.degreeShort as string,
					).toLowerCase(),
			)
			.map(([k, _v]) => k);
	}
	...
	private async discoverValidDegrees(): Promise<ProgramCodes[]> {
		const [awardTypes, reports] = await Promise.all([
			fetch("https://www.reg.uci.edu/mdsd/api/lookups/awardTypes").then((r) =>
				r.json(),
			),
			...
			.filter(
				(ent) =>
					...
					(!ent.major.endTermYyyyst ||
						...
						new Date(
							`${ent.major.endTermYyyyst.slice(1)}-01-01`,
						).getUTCFullYear() >= 2006) &&
	...
	private async scrapePrograms(degrees: Iterable<ProgramCodes>) {
		...
		for (const degree of degrees) {
			...
			if (ret.has(this.asMajorSpecId(majorAudit.title, specCode))) {
				console.log(
					`Requirements block already exists for "${majorAudit.title}" ${majorLogInfo}`,
				);
				continue;
			}
			...
			console.log(
				`Requirements block found and parsed for "${majorAudit.title}" ${majorLogInfo}`,
			);
		}
		return ret;
	}
	 ...
	 * @private
	 */
	private specializationParentCandidates(
		specCode: string,
	): DegreeWorksProgram[] {
		...
		if (specCode === "OACSC") {
			...
			const inMap = this.parsedPrograms.get(
				this.asMajorSpecId("Major in Chemistry", undefined),
			);
	...
	async run() {
		if (this.done)
			throw new Error("This scraper instance has already finished its run.");
		...
		console.log(
			"Fetched university, GE, and attempted to fetch honors requirements (see above)",
		);
		...
		const majorToCollegeCode = new Map(
			validDegrees.map(({ majorCode, collegeCode }) => [
				majorCode,
				collegeCode,
			]),
		);
		...
		console.log(
			`loading ${this.specializationCache.size} cached specializations`,
		);
		...
		for (const [specCode, specName] of this.knownSpecializations.entries()) {
			...
			if (this.specializationCache.has(specCode)) {
				...
				const got = this.specializationCache.get(
					specCode,
				) as SpecializationCache | null;
				newlyResolved = false;
			...
			if (newlyResolved && (!specBlock || !foundMajor)) {
				...
				for (const candidate of majorCandidates) {
					if (!candidate.degreeType)
						throw new Error("Degree type is undefined");
			...
			if (newlyResolved) {
				...
				await fs.writeFile(
					specCacheFilename,
					JSON.stringify(
						Object.fromEntries(this.specializationCache),
						undefined,
						4,
					),
				);
		...
		for (const [majorSpecId, majorProgram] of await this.scrapePrograms(
			foundMajorSpecPairs,
		)) {
		...
		const x = this.parsedPrograms.get(
			this.asMajorSpecId("Major in Art History", undefined),
		);
		...
		if (x && y && z) {
			...
			this.parsedPrograms.delete(
				this.asMajorSpecId("Major in Art History", "AHPER"),
			);
			this.parsedPrograms.delete(
				this.asMajorSpecId("Major in Art History", "AHGEO"),
			);
			this.parsedPrograms.set(
				this.asMajorSpecId("Major in Art History", undefined),
				x,
			);
		}
	...
	get() {
		if (!this.done)
			throw new Error("This scraper instance has not yet finished its run.");
		return {
	...
	static async new(meta: {
		...
		const studentId = jwtDecode<JwtPayload>(
			authCookie.slice(JWT_HEADER_PREFIX_LENGTH),
		)?.sub;
