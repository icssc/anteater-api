...
export const createKeySchema = z
	.object({
		...
		resources: z
			.record(z.enum(accessControlledResources), z.boolean())
			.optional(),
	})
	.strict();

export const createRefinedKeySchema = createKeySchema.superRefine(
	(data, ctx) => {
		if (data._type === "publishable") {
			if (!data.origins || data.origins.length === 0) {
				...
				data.origins.forEach((origin, index) => {
					if (
						!origin.url.startsWith("http://") &&
						!origin.url.startsWith("https://")
					) {
						ctx.issues.push({
							input: origin.url,
							code: "custom",
	...
	return {
		...
		origins:
			data._type === "publishable"
				? (Object.fromEntries(
						data.origins?.map((origin: z.infer<typeof originSchema>) => [
							origin.url,
							true,
						]) ?? [],
