...
const isValidRestrictionCode = (
	code: string,
): code is (typeof restrictionCodes)[number] =>
...
export const websocQuerySchema = z.object({
	...
	quarter: z.enum(terms, {
		error: (issue) =>
			issue.input === undefined
				? "Parameter 'quarter' is required"
				: "Invalid parameter 'quarter'",
	}),
	ge: z
		.enum(geCategories)
		.optional()
	...
	department: z.string().optional().openapi({
		description:
			"Only include courses offered by the specified department code",
		example: "I&C SCI",
	}),
	...
	sectionCodes: z
		...
		.openapi({
			description:
				"A comma-separated list of section codes or section code ranges",
			example: "36210,36216-36218",
		}),
	instructorName: z.string().optional().openapi({
		description:
			"Only include courses taught by the specified instructor (case-insensitive)",
		example: "WONG-MA, J.",
	}),
	...
	building: z.string().optional().openapi({
		description:
			"Only include sections which have at least one meeting in the specified building",
		example: "PSLH",
	}),
	room: z.string().optional().openapi({
		description:
			"Only include sections which have at least one meeting in a room with this number",
		example: "100",
	}),
	division: z
		.enum(courseLevels)
...
export const syllabiQuerySchema = z.object({
	...
	quarter: z
		.enum(terms, { error: (_issue) => "Invalid quarter provided" })
		.optional(),
	instructor: z.string().optional().openapi({
		description:
			"Only include syllabi for sections taught by the specified instructor",
		example: "SHINDLER, M.",
	}),
});
