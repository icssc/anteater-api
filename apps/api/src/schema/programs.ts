...
const programIdBase = z.string({
	error: (issue) =>
		issue.input === undefined ? "programId is required" : "invalid programId",
});
...
export const specializationsQuerySchema = z.object({
	majorId: z.string().optional().openapi({
		description:
			"Only fetch specializations associated with the major with this ID, if provided",
		example: "BS-201",
	}),
});
...
export const ugradRequirementsQuerySchema = z.object({
	id: z
		.enum(UgradRequirementsBlockIds)
...
export const programCourseRequirementSchema = programRequirementBaseSchema
	.extend({
		...
		courseCount: z.number().int().nonnegative().openapi({
			description:
				"The number of courses from this set demanded by this requirement.",
		}),
...
> = programRequirementBaseSchema
	...
	.openapi({
		description:
			"A group requirement; a requirement to fulfill some number of sub-requirements.",
		example: {
...
export const majorsResponseSchema = z.array(
	z.object({
		...
		specializationRequired: z.boolean().openapi({
			description:
				"Whether a specialization must be completed to complete this degree",
		}),
...
export const majorRequirementsResponseSchema =
	programRequirementsResponseSchema.extend({
		...
		schoolRequirements: z
			.object({
				name: z
					.string()
					.openapi({ description: "Name for this school's requirements" }),
				requirements: programRequirementsResponseSchema.shape.requirements,
			})
			.nullable()
			.openapi({
				description:
					"If present, requirements imposed by the school this major belongs to",
			}),
	});

export const minorRequirementsResponseSchema =
	programRequirementsResponseSchema.extend({
...
export const specializationRequirementsResponseSchema =
	programRequirementsResponseSchema.extend({
		id: programRequirementsResponseSchema.shape.id.openapi({
			example: "BS-201E",
		}),
...
export const ugradRequirementsResponseSchema = z.object({
	id: z
		.string()
		.openapi({ description: "ID of the requirements block fetched" }),
	requirements: z
