...
export const eventSchema = z.object({
	...
	image: z
		.string()
		.nullable()
		.openapi({ description: "URL to event promotional image" }),
	...
	start: z.coerce.date().openapi({
		...
		description:
			"End date of event. Events with no end date are often one-day events.",
	}),
...
export const restaurantsQuerySchema = z.object({
	id: restaurantIdSchema.optional().openapi({
		description:
			"If present, only return the restaurant with this ID (if it exists)",
	}),
});
...
export const restaurantTodayResponseSchema = restaurantSchema.extend({
	periods: z.record(
		...
		z.object({
			name: z
				.string()
				.openapi({ description: "The name of a period", example: "Lunch" }),
			startTime: z.iso.time().nullable().openapi({
				description:
					"The start time of a period, or null if not served on this day",
				example: "11:00:00",
			}),
			endTime: z.iso.time().nullable().openapi({
				description:
					"The end time of a period, or null if not served on this day",
				example: "14:30:00",
			}),
			stationToDishes: z.record(
				...
				z.string().array().openapi({
					description:
						"The ID(s) of the dish(es) served at this station in this period.",
				}),
			),
			updatedAt: z.date(),
		}),
