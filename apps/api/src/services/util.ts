...
export function buildCourseLevelQuery(
	...
	if (courseLevel) {
		switch (courseLevel) {
			case "LowerDiv":
				conditions.push(
					and(gte(table.courseNumeric, 1), lte(table.courseNumeric, 99)),
				);
				break;
			case "UpperDiv":
				conditions.push(
					and(gte(table.courseNumeric, 100), lte(table.courseNumeric, 199)),
				);
				break;
			case "Graduate":
			...
			switch (num._type) {
				case "ParsedInteger":
					courseNumberConditions.push(
						eq(websocCourse.courseNumeric, num.value),
					);
					break;
				case "ParsedString":
				...
				case "ParsedRange":
					courseNumberConditions.push(
						and(
							gte(websocCourse.courseNumeric, num.min),
							lte(websocCourse.courseNumeric, num.max),
						),
					);
					break;
			}
