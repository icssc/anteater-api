...
type InstructorsByCursorServiceInput = z.infer<
	typeof instructorsByCursorQuerySchema
>;

function buildQuery(
	input: InstructorsServiceInput | InstructorsByCursorServiceInput,
) {
	const conditions = [];
	...
	if (input.departmentContains) {
		conditions.push(
			ilike(instructorView.department, `%${input.departmentContains}%`),
		);
	}
...
export class InstructorsService {
	...
	async getInstructorByUCInetID(
		ucinetid: string,
	): Promise<InstructorsServiceOutput | null> {
		return this.getInstructorsRaw({
