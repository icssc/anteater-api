			...
			switch (code._type) {
				case "ParsedInteger":
					sectionCodesConditions.push(
						eq(websocSection.sectionCode, code.value),
					);
					break;
				case "ParsedRange":
					sectionCodesConditions.push(
	...
	if (input.fullCourses) {
		switch (input.fullCourses) {
			...
			case "SkipFullWaitlist":
				conditions.push(
					and(
						ne(websocSection.status, "FULL"),
						ne(websocSection.status, "Waitl"),
					),
				);
				break;
			case "FullOnly":
				conditions.push(
					or(
						eq(websocSection.status, "FULL"),
						eq(websocSection.status, "Waitl"),
					),
				);
				break;
			case "Overenrolled":
				conditions.push(
					or(
						gt(
							websocSection.numCurrentlySectionEnrolled,
							websocSection.maxCapacity,
						),
						gt(
							websocSection.numCurrentlyTotalEnrolled,
							websocSection.maxCapacity,
						),
					),
				);
...
const transformSection = (
	section: Row["section"],
): z.infer<typeof websocSectionSchema> => {
	...
	return {
		...
		numCurrentlyEnrolled: {
			totalEnrolled:
				negativeAsNull(section.numCurrentlyTotalEnrolled)?.toString(10) ?? "",
			sectionEnrolled:
				negativeAsNull(section.numCurrentlySectionEnrolled)?.toString(10) ?? "",
		},
		numNewOnlyReserved:
			negativeAsNull(section.numNewOnlyReserved)?.toString(10) ?? "",
...
export class WebsocService {
	...
	makeSelect(
		...
		const base = this.db
			...
			.innerJoin(
				websocDepartment,
				eq(websocSchool.id, websocDepartment.schoolId),
			)
			.innerJoin(
				websocCourse,
				eq(websocDepartment.id, websocCourse.departmentId),
			)
		...
		return base
			...
			.leftJoin(
				websocSectionMeeting,
				eq(websocSection.id, websocSectionMeeting.sectionId),
			)
			.leftJoin(
				websocSectionMeetingToLocation,
			...
			.leftJoin(
				websocLocation,
				eq(websocLocation.id, websocSectionMeetingToLocation.locationId),
			);
	}
	...
	async getSyllabi(input: z.infer<typeof syllabiQuerySchema>) {
		const conditions = [
			eq(websocCourse.courseId, input.courseId),
			ne(websocSection.webURL, ""),
		];
		if (input.year) {
		...
		if (input.instructor) {
			conditions.push(
				eq(websocSectionToInstructor.instructorName, input.instructor),
			);
		}
		return this.db
			.select({
