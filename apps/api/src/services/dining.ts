...
export class DiningService {
	...
	async getUpcomingEvents(input: DiningEventQuery) {
		...
		const conds = [
			or(
				...
				and(
					isNull(diningEvent.end),
					gte(diningEvent.updatedAt, sql`NOW() - INTERVAL '2 WEEKS'`),
				),
			),
		];
	...
	async getDishesRaw(input: {
		where?: SQL;
	}): Promise<z.infer<typeof dishSchema>[]> {
		const rows = await this.db
			.select({
				...
				nutritionInfo: {
					...
					updatedAt: diningNutritionInfo.updatedAt,
				},
				dietRestriction: {
			...
			.leftJoin(
				diningNutritionInfo,
				eq(diningDish.id, diningNutritionInfo.dishId),
			)
			.leftJoin(
				diningDietRestriction,
				eq(diningDish.id, diningDietRestriction.dishId),
			)
			.where(input.where);
	...
	async getDishById(input: DiningDishQuery) {
		return this.getDishesRaw({ where: eq(diningDish.id, input.id) }).then((x) =>
			orNull(x[0]),
		);
	}
	...
	async getRestaurants(query: z.infer<typeof restaurantsQuerySchema>) {
		const fetched = await this.db
			...
			.leftJoin(
				diningStation,
				eq(diningRestaurant.id, diningStation.restaurantId),
			)
	...
	async getRestaurantToday(query: z.infer<typeof restaurantTodayQuerySchema>) {
		const rows = await this.db
			...
			.leftJoin(
				diningPeriod,
				eq(diningRestaurant.id, diningPeriod.restaurantId),
			)
			.leftJoin(
				diningStation,
				eq(diningRestaurant.id, diningStation.restaurantId),
			...
			.leftJoin(
				diningDishToPeriod,
				eq(diningPeriod.id, diningDishToPeriod.periodId),
			)
			.leftJoin(diningDish, eq(diningDish.id, diningDishToPeriod.dishId))
			.where(
				and(
					eq(diningRestaurant.id, query.id),
					eq(diningPeriod.date, query.date),
				),
			)
			.groupBy(
		...
		type PeriodsRecord = z.infer<
			typeof restaurantTodayResponseSchema
		>["periods"];
		...
		for (const { period, station, dishes } of rows) {
			...
			if (station !== null) {
				getFromMapOrThrow(periods, period.id).stationToDishes[station.id] =
					dishes;
			}
		}
