...
function buildQuery(input: GradesServiceInput) {
	...
	if (input.sectionCode) {
		conditions.push(
			eq(websocSection.sectionCode, Number.parseInt(input.sectionCode, 10)),
		);
	}
...
export class GradesService {
	...
	async getRawGrades(input: GradesServiceInput) {
		return this.db
			...
			.innerJoin(
				websocSectionGrade,
				eq(websocSectionGrade.sectionId, websocSection.id),
			)
			.leftJoin(
				websocSectionToInstructor,
							...
							acc.has(row.id)
								...
								: acc.set(row.id, {
										...
										averageGPA: row.averageGPA
											? Number.parseFloat(row.averageGPA)
											: null,
	...
	async getAggregateGrades(input: GradesServiceInput) {
		const sectionMapping = await this.db
			...
			.then((rows) =>
				rows.reduce(
					...
					new Map<
						string,
						z.infer<typeof aggregateGradesSchema>["sectionList"][number]
					>(),
				),
			);
		...
		const [gradeDistribution] = await this.db
			...
			.where(
				inArray(websocSectionGrade.sectionId, sectionMapping.keys().toArray()),
			);
		return {
