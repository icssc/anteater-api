...
type EnrollmentHistoryServiceInput = z.infer<
	typeof enrollmentHistoryQuerySchema
>;
...
function transformSectionRows(
	...
	const mapping = new Map<
		string,
		Omit<
			z.infer<typeof enrollmentHistorySchema>,
	...
	async getEnrollmentHistory(input: EnrollmentHistoryServiceInput) {
		...
		const enrollmentRows = await this.db
			...
			.where(
				inArray(
					...
					transformedSectionRows.keys().toArray(),
				),
			)
		...
		for (const row of enrollmentRows) {
			...
			if (section) {
				...
				section.totalEnrolledHistory.push(
					row.numCurrentlyTotalEnrolled?.toString(10) ?? "",
				);
				...
				section.newOnlyReservedHistory.push(
					row.numNewOnlyReserved?.toString(10) ?? "",
				);
				section.statusHistory.push(row.status ?? "");
			}
		}
		const filteredSections = transformedSectionRows
			.values()
			.filter((section) => {
