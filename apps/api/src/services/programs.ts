...
export class ProgramsService {
		...
		const majorSpecialization = this.db.$with("major_specialization").as(
			this.db
				.select({
					...
					name: major.name,
					specializations:
						sql`ARRAY_REMOVE(ARRAY_AGG(${specialization.id}), NULL)`.as(
							"specializations",
						),
				})
				.from(major)
	...
	async getSpecializations(query: z.infer<typeof specializationsQuerySchema>) {
		return this.db
			...
			.from(specialization)
			.where(
				query.majorId ? eq(specialization.majorId, query.majorId) : undefined,
			);
	}

	async getMajorRequirements(
		query: z.infer<typeof majorRequirementsQuerySchema>,
	) {
		return await this.getProgramRequirements({ programType: "major", query });
	}

	async getMinorRequirements(
	...
	private async getProgramRequirements({
		...
		if (programType === "major") {
			const [got] = await this.db
				...
				.leftJoin(
					collegeRequirement,
					eq(major.collegeRequirementId, collegeRequirement.id),
				)
				.leftJoin(
					majorSpecializationToRequirement,
				...
				.leftJoin(
					majorRequirement,
					eq(
						majorSpecializationToRequirement.requirementId,
						majorRequirement.id,
					),
				)
				.limit(1);
	...
	async getUgradRequirements(
		query: z.infer<typeof ugradRequirementsQuerySchema>,
	) {
		const [got] = await this.db
			.select({
	...
	async getSamplePrograms(query: z.infer<typeof sampleProgramsQuerySchema>) {
        ...
        `.as("variations"),
			...
			.leftJoin(
				sampleProgramVariation,
				eq(catalogProgram.id, sampleProgramVariation.programId),
			)
