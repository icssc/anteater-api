...
): (typeof outputCourseLevels)[number] =>
...
function buildQuery(input: CoursesServiceInput | CoursesByCursorServiceInput) {
	...
	if (input.courseLevel) {
		switch (input.courseLevel) {
			case "LowerDiv":
				conditions.push(
					and(
						gte(courseView.courseNumeric, 0),
						lt(courseView.courseNumeric, 100),
					),
				);
				break;
			case "UpperDiv":
				conditions.push(
					and(
						gte(courseView.courseNumeric, 100),
						lt(courseView.courseNumeric, 200),
					),
				);
				break;
			case "Graduate":
	...
	conditions.push(
		...buildUnitBoundsQuery(courseView, input.minUnits, input.maxUnits),
	);
	if (input.descriptionContains) {
		conditions.push(
			ilike(courseView.description, `%${input.descriptionContains}%`),
		);
	}
	if (input.geCategory) {
...
export class CoursesService {
	...
	async getCourseById(id: string): Promise<CoursesServiceOutput | null> {
		return this.getCoursesRaw({ where: eq(courseView.id, id) }).then((x) =>
			orNull(x[0]),
		);
	}

	async getCourses(
		input: CoursesServiceInput,
	): Promise<CoursesServiceOutput[]> {
		return this.getCoursesRaw({
	...
	async getCoursesByCursor(
		...
		const nextCursor =
			input.take === 0
				? null
				: courses.length > input.take
					? courses[input.take].id
					: null;
