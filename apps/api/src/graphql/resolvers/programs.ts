...
export const programResolvers = {
	Query: {
		...
		specialization: async (
			...
			const parsedArgs = specializationRequirementsQuerySchema.parse(
				args?.query,
			);
			const service = new ProgramsService(db);
			...
			if (!res)
				throw new GraphQLError(
					`Specialization ${parsedArgs.programId} not found`,
					{
						extensions: { code: "NOT_FOUND" },
					},
				);
	...
	ProgramRequirement: {
		...
		__resolveType: (x: {
			requirementType: z.infer<
				typeof programRequirementSchema
			>["requirementType"];
		}) => {
