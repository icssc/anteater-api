	...
	Query: {
		diningEvents: async (
			...
			return await service.getUpcomingEvents(
				diningEventsQuerySchema.parse(args.query ?? {}),
			);
		},
		diningDish: async (
			_: unknown,
		...
		getRestaurants: async (
			...
			const service = new DiningService(db);
			return await service.getRestaurants(
				restaurantsQuerySchema.parse(args.query ?? {}),
			);
		},
		getRestaurantToday: async (
			_: unknown,
			args: { query: unknown },
			{ db }: GraphQLContext,
		) => {
			const service = new DiningService(db);
			const data = await service.getRestaurantToday(
				restaurantTodayQuerySchema.parse(args.query),
			);
			if (!data) {
